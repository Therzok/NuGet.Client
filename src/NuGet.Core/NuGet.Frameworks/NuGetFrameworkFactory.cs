        ...
        public static NuGetFramework ParseFrameworkName(string frameworkName, IFrameworkNameProvider mappings)
        {
            ...
            if (!TryParseSpecialFramework(parts[0], out result))
            {
                    ...
                    && StringComparer.OrdinalIgnoreCase.Equals(FrameworkConstants.FrameworkIdentifiers.NetCoreApp, framework))
                {
                    ...
                    var platformPart = SingleOrDefaultSafe(parts.Where(s => s.IndexOf("Platform=", StringComparison.OrdinalIgnoreCase) == 0));
                    var platformVersionPart = SingleOrDefaultSafe(parts.Where(s => s.IndexOf("PlatformVersion=", StringComparison.OrdinalIgnoreCase) == 0));

                    if (!string.IsNullOrEmpty(platformPart))
                    {
                        platform = platformPart.Split('=')[1];
                    }

                    if (!string.IsNullOrEmpty(platformVersionPart))
                    {
                        var versionString = platformVersionPart.Split('=')[1].TrimStart('v');
                            ...
                            || mappings.TryGetVersion(parts.Item2, out version))
                        {
                                ...
                                && StringComparer.OrdinalIgnoreCase.Equals(FrameworkConstants.FrameworkIdentifiers.Net, framework))
                            {
                                ...
                                if (!string.IsNullOrEmpty(profileShort))
                                {
                                    ...
                                    mappings.TryGetVersion(platformVersionString, out platformVersion);

                                    if (mappings.TryGetPlatform(framework, platformString, out platform))
                                    {
                                        result = new NuGetFramework(framework, version, platform, platformVersion);
                                    }
                                    else
